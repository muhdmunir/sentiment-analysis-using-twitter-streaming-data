...
class StdOutListener(tweepy.StreamListener):
    ...
    def on_data(self, data):
        ...
        tweets_csv = csv.writer(open("covid19.csv",mode="a")) # save in csv format

        tweets_text = open("covid19.txt","a") # save in txt format


        if not os.path.exists('covid19.json'):

            json1_file = open("covid19.json" , 'w')

            json.dump([tweets], json1_file)

        else:

            json1_read = open('covid19.json' , 'r')
            ...
            with open('covid19.json', 'w+') as my_file:
